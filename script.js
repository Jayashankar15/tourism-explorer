...
// Add these fixes to your script.js file

// 1. Add null checks to all DOM element access
function setupForms() {
    setupTripPlanForm();
    setupReviewForm();
}

// Trip planning form with null checks
function setupTripPlanForm() {
    const form = document.getElementById('tripPlanForm');
...
// Review form handling with null checks
function setupReviewForm() {
    const form = document.getElementById('reviewForm');
    if (!form) return; // Exit if form doesn't exist
    ...
    form.addEventListener('submit', function (e) {
        ...
        // Handle photo upload if element exists
        if (photoEl && photoEl.files && photoEl.files[0]) {
            const file = photoEl.files[0];
            const reader = new FileReader();
            reader.onload = () => {
...
// Display reviews with null checks
function displayReviews() {
    const reviewsList = document.getElementById('reviewsList');
    if (!reviewsList) return; // Exit if element doesn't exist
...
// Navigation setup with null checks
function setupNavigation() {
    const hamburger = document.querySelector('.hamburger');
    const navMenu = document.querySelector('.nav-menu');
    const navLinks = document.querySelectorAll('.nav-link');
...
function initializeFilters() {
    ...
    if (!applyFiltersBtn) return; // Element doesn't exist on this page
    ...
    applyFiltersBtn.addEventListener('click', () => {
        ...
        if (window.location.pathname.includes('restaurants') || window.location.pathname.includes('restaurantspage')) {
            url = 'restaurantspage.html';
...
// Hotel/Restaurant filtering with null checks
